...
def _same_file(a: Path, b: Path) -> bool:
    """True if two paths point at the same bytes (size match is enough here)."""
    try:
        return a.samefile(b) or a.stat().st_size == b.stat().st_size
    except OSError:
        return False
...
class WeightsManager:
    ...
    def list(self, refresh: bool = False) -> list[dict]:
        """Merged catalog view. Each item:
        {file, label, size_mb, downloaded, downloading, source, available, coming_soon}.

        The known catalog (Base/FinePatch/Deep/Wide) is ALWAYS included so an
        announced-but-not-yet-uploaded variant shows as a transparent "coming soon"
        placeholder; once it lands on the Hub it flips to downloadable on its own,
        with no app change. Locally imported checkpoints appear as source="custom".
        """
        remote = self._remote_sizes(refresh=refresh)
        ckpt_dir = checkpoints_dir()
        local = {p.name for p in ckpt_dir.glob("*.pth") if p.is_file()}
        sizes: dict[str, int | None] = dict(remote) if remote else {}
        names = _ordered(set(_KNOWN_FILES) | set(sizes) | local)
        with self._lock:
        ...
        for name in names:
            ...
            on_remote = name in sizes
            is_known = name in _KNOWN_FILES
            ...
            if size_b is None and downloaded:
                try:
                    ...
                    size_b = None
            # source: official Hub catalog entry, or a user-imported checkpoint.
            source = "custom" if (downloaded and not is_known and not on_remote) else "hf"
            # coming_soon: announced (known) but not on the Hub yet and not local.
            coming_soon = is_known and not on_remote and not downloaded
            items.append(
                {
                    "file": name,
                    "label": _label(name),
                    ...
                    "source": source,
                    "available": on_remote,       # downloadable from the Hub right now
                    "coming_soon": coming_soon,
                }
            )
        return items

    def import_local(self, src_path: str) -> str:
        """Copy a user-provided .pth into the checkpoints dir; return its stored name.

        WaveDiT-structure validation happens in the engine after this returns; here
        we only guard the path/extension and avoid clobbering an existing file.
        """
        src = Path(src_path).expanduser()
        if not src.is_file():
            raise ValueError(f"file not found: {src_path}")
        if src.suffix.lower() != ".pth":
            raise ValueError("only .pth checkpoints can be imported")
        ckpt_dir = checkpoints_dir()
        stem = re.sub(r"[^A-Za-z0-9._-]", "_", src.stem).strip("._-") or "model"
        name = f"{stem}.pth"
        _check_name(name)
        with self._lock:
            busy = set(self._downloading)
        if name in busy:
            raise ValueError(f"{name} is currently downloading")
        target = ckpt_dir / name
        # Never overwrite an existing checkpoint: pick the next free suffix.
        i = 2
        while target.exists() and not _same_file(src, target):
            name = f"{stem}-{i}.pth"
            target = ckpt_dir / name
            i += 1
        if not target.exists():
            tmp = ckpt_dir / (name + ".part")
