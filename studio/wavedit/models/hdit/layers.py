"""HDiT layers: conditioned norms, attention, resampling, embeddings and U-Net scaffolding.

Adapted and extended to 3D from k-diffusion (https://github.com/crowsonkb/k-diffusion).
The original Karras ``Denoiser`` / DCT frequency-weighting utilities are not used by
WaveDiT (which trains its own flow-matching objective) and have been removed; this also
drops the optional ``dctorch`` dependency.
"""
