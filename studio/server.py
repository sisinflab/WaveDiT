...
# Set by main.py in window mode: () -> chosen .pth path or None on cancel (native
# open dialog for importing a custom checkpoint).
import_handler: Callable[[], str | None] | None = None
...
class StudioHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802 - http.server naming
        ...
        routes: dict[str, Callable[[dict[str, Any]], Any]] = {
            ...
            "/api/weights/import": self._post_weights_import,
    ...
    def _post_weights_import(self, body: dict[str, Any]) -> dict[str, Any]:
        """Import a local WaveDiT checkpoint via the native open dialog.

        Body may carry an explicit 'path' (used headlessly / in tests); otherwise the
        desktop import_handler opens a file picker. The file is copied in, then
        validated as a real WaveDiT checkpoint; an invalid file is removed and rejected.
        """
        src = body.get("path")
        if not src:
            if import_handler is None:
                raise ApiError(400, "import is only available in the desktop app")
            src = import_handler()
            if not src:
                return {"cancelled": True}
        if not isinstance(src, str):
            raise ApiError(400, "invalid import path")
        name = weights.import_local(src)
        try:
            label = _engine().validate_checkpoint(name)
        except ValueError as exc:
            weights.delete(name)
            raise ApiError(400, f"not a WaveDiT checkpoint: {exc}") from exc
