...
(() => {
  ...
  const store = {
    ...
    weights: [],
    library: [],
    params: { model: null, age: 72, seed: 42, steps: 10, cfg_scale: 1.0, sampler: "heun", morpheus: 1.0 },
    lastSeed: null,
    colormap: "gray",
    gamma: 1.0,
    busy: false,
  ...
  /* ======================================================================
     ...
     ====================================================================== */
  const modelCaption = (w) => (w.label === "Base" ? "fast" : w.label === "FinePatch" ? "detailed" : (w.size_mb ? fmtMB(w.size_mb) : "checkpoint"));
  ...
  function renderModelSeg() {
    ...
    for (const w of store.weights) {
      ...
      if (w.downloading) cap.textContent = "downloading…";
        ...
        cap.innerHTML = '<svg class="dl-glyph" viewBox="0 0 16 16" aria-hidden="true"><path d="M8 2v7M5.4 6.6 8 9.2l2.6-2.6M3 12.5h10" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg> download';
  ...
  function applyViewerLook() {
    ...
    try {
      ...
      nv.setClipPlane(CLIP_PLANE.slice());
    ...
    try {
      ...
      while (cur) {
        ...
        try {
          ...
          if (NVC) nv.setSliceType(NVC.SLICE_TYPE.MULTIPLANAR);
          applyViewerLook();
        } catch (e) {
  ...
  function renderWeightsLists() {
    document.querySelectorAll('[data-role="weights-list"]').forEach((list) => {
      ...
      if (!store.weights.length) {
        const p = document.createElement("p");
        p.className = "microcopy";
        p.textContent = "No checkpoints listed. Check the network connection and reopen this panel.";
        list.appendChild(p);
        return;
      }
      for (const w of store.weights) list.appendChild(makeWeightCard(w));
    });
  }
  ...
  function makeWeightCard(w) {
    ...
    status.textContent = w.downloading ? "downloading…"
      : w.downloaded ? `downloaded${w.size_mb ? ` · ${fmtMB(w.size_mb)}` : ""}`
      : `not downloaded${w.size_mb ? ` · ${fmtMB(w.size_mb)}` : ""}`;
    ...
    if (w.downloaded) {
      ...
      on(del, "click", async () => {
        ...
        const sure = await confirmDialog({
          title: `Delete ${w.label} weights?`,
          message: "The checkpoint can always be downloaded again from Hugging Face.",
        });
        ...
        try {
          ...
          toast(`${w.label} weights deleted`, "success");
        } catch (e) {
          toast(`Delete failed: ${e.message}`, "error");
        }
        refreshWeights();
  ...
  function connectSSE() {
    ...
    onEv("gen_error", (d) => {
      ...
      if (!store.library.length) byId("empty-state").hidden = false;
      softStateRefresh();
    });
  ...
  async function init() {
    ...
    try {
      ...
      else byId("empty-state").hidden = false;
    } catch (e) {
