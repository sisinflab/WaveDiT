...
Entry-point design
------------------
PyInstaller analyzes macos/studio/main.py as the entry *script*: in the frozen
app it runs as __main__, exactly like `python studio/main.py` in development.
The rest of the `studio` package and the vendored `wavedit` package are bundled
as importable modules through explicit hiddenimports plus pathex:

  * pathex includes macos/        -> `import studio.<mod>` resolves
  * pathex includes macos/studio  -> `import wavedit.<mod>` resolves, because the
...
MACOS_DIR = SPEC_DIR.parent           # .../macos
STUDIO_DIR = MACOS_DIR / "studio"
