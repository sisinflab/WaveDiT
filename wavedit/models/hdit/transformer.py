"""Largely Extended to 3D from https://github.com/crowsonkb/k-diffusion"""
...
from .neighborhood_attention import na2d as neighborhood_attention_2d
...
class AdaRMSNorm(nn.Module):
    ...
    def forward(self, x, cond):
        # Calculate the scale from the conditioning signal
        scale = self.linear(cond) + 1

        # Adapt the scale's shape to the input tensor's dimensionality for correct broadcasting
        if x.ndim == 4:  # For 4D image-like tensors: (Batch, Height, Width, Channels)
            scale = scale[:, None, None, :]
        elif x.ndim == 3:  # For 3D token sequences: (Batch, NumTokens, Channels)
            scale = scale[:, None, :]
        elif x.ndim == 2:  # For 2D features: (Batch, Channels)
            pass  # No unsqueezing needed as scale shape (Batch, Channels) already matches
        else:
            # Raise an error for unsupported tensor shapes to prevent silent failures.
            raise ValueError(f"AdaRMSNorm received an input tensor `x` with an unsupported number of dimensions: {x.ndim}")
...
class NeighborhoodSelfAttentionBlock(nn.Module):
    ...
    def forward(self, x, pos, cond):
        ...
        x = neighborhood_attention_2d(q, k, v, self.kernel_size, scale=1.0)
...
class FeedForwardBlock(nn.Module):
    ...
    def forward(self, x, cond):
        ...
        x = self.dropout(x)
        x = self.down_proj(x)
        return skip + x * torch.sigmoid(self.gate)


class SpatioTemporalAttentionBlock(nn.Module):
    """
    An efficient attention block that factorizes attention spatio-temporally.
    1. Spatial Attention: Attends within each 2D slice.
    2. Temporal (Depth) Attention: Attends across the depth dimension at each spatial token location.
    This is inspired by video transformer architectures.
    """
    def __init__(self, d_model, d_head, cond_features, dropout=0.0):
        super().__init__()
        self.d_head = d_head
        ...
        # --- Layers for Spatial Attention (within each slice) ---
        self.norm_spatial = AdaRMSNorm(d_model, cond_features)
        self.qkv_proj_spatial = apply_wd(Linear(d_model, d_model * 3, bias=False))
        self.scale_spatial = nn.Parameter(torch.full([self.n_heads], 10.0))
        self.pos_emb = AxialRoPE(d_head // 2, self.n_heads) # 2D RoPE for spatial
        self.out_proj_spatial = apply_wd(zero_init(Linear(d_model, d_model, bias=False)))
        
        # --- Layers for Temporal (Depth-wise) Attention ---
        self.norm_depth = AdaRMSNorm(d_model, cond_features)
        self.qkv_proj_depth = apply_wd(Linear(d_model, d_model * 3, bias=False))
        self.out_proj_depth = apply_wd(zero_init(Linear(d_model, d_model, bias=False)))
    ...
    def forward(self, x, pos, cond, d_c):
        if d_c is None:
            raise ValueError("SpatioTemporalAttentionBlock requires d_c (depth of the volume).")

        B_slice, H, W, C = x.shape
        B = B_slice // d_c
        
        # 1. SPATIAL ATTENTION (operates on the batch of slices)
        skip_spatial = x
        x_spatial = self.norm_spatial(x, cond)
        qkv_spatial = self.qkv_proj_spatial(x_spatial)
        
        q, k, v = rearrange(qkv_spatial, "b h w (t nh e) -> t b nh (h w) e", t=3, e=self.d_head)
        
        pos_flat = rearrange(pos, "... h w e -> ... (h w) e").to(q.dtype)
        theta = self.pos_emb(pos_flat).movedim(-2, -3)
        
        q, k = scale_for_cosine_sim(q, k, self.scale_spatial[:, None, None], 1e-6)
        q = apply_rotary_emb_(q, theta)
        k = apply_rotary_emb_(k, theta)
        
        attn_out_spatial = F.scaled_dot_product_attention(q, k, v, scale=1.0)
        attn_out_spatial = rearrange(attn_out_spatial, "b nh (h w) e -> b h w (nh e)", h=H, w=W)
        attn_out_spatial = self.dropout(attn_out_spatial)
        x_after_spatial = skip_spatial + self.out_proj_spatial(attn_out_spatial)

        # 2. TEMPORAL (DEPTH-WISE) ATTENTION
        skip_depth = x_after_spatial
        
        # Reshape for depth attention: group slices back into volumes
        # [B*d_c, H, W, C] -> [B, d_c, H, W, C]
        x_vol = x_after_spatial.view(B, d_c, H, W, C)
        # Permute to make depth the sequence dimension for each spatial token
        # [B, H, W, d_c, C]
        x_permuted = x_vol.permute(0, 2, 3, 1, 4).contiguous()
        # Create a batch of depth sequences: [B*H*W, d_c, C]
        x_depth_sequences = x_permuted.view(B * H * W, d_c, C)

        # The conditioning signal `cond` is per-slice. We need one for each new batch item.
        # Reshape cond to [B, d_c, C_cond] and select one representative cond per volume from the middle slice
        cond_reshaped = cond.view(B, d_c, -1)
        cond_for_depth = cond_reshaped[:, d_c // 2, :] # [B, C_cond]
        # Repeat for each spatial token location
        cond_for_depth = cond_for_depth.repeat_interleave(H * W, dim=0) # [B*H*W, C_cond]

        x_depth_norm = self.norm_depth(x_depth_sequences, cond_for_depth)
        qkv_depth = self.qkv_proj_depth(x_depth_norm)
        
        q_d, k_d, v_d = rearrange(qkv_depth, "b s (t nh e) -> t b nh s e", t=3, e=self.d_head)
        attn_out_depth = F.scaled_dot_product_attention(q_d, k_d, v_d, scale=None) # Use default scaling
        attn_out_depth = rearrange(attn_out_depth, "b nh s e -> b s (nh e)", s=d_c)
        attn_out_depth = self.dropout(attn_out_depth)
        attn_out_depth = self.out_proj_depth(attn_out_depth) # [B*H*W, C]

        # Reshape back to original slice format
        attn_out_depth_vol = attn_out_depth.view(B, H, W, d_c, C)
        attn_out_depth_permuted = attn_out_depth_vol.permute(0, 3, 1, 2, 4).contiguous()
        final_attn_out = attn_out_depth_permuted.view(B_slice, H, W, C)
        
        return skip_depth + final_attn_out
...
class GlobalTransformerLayer(nn.Module):
    ...
    def forward(self, x, pos, cond, d_c=None): # Added d_c for consistent signature
...
class SpatioTemporalTransformerLayer(nn.Module):
    def __init__(self, d_model, d_ff, d_head, cond_features, dropout=0.0):
        super().__init__()
        self.self_attn = SpatioTemporalAttentionBlock(d_model, d_head, cond_features, dropout=dropout)
        self.ff = FeedForwardBlock(d_model, d_ff, cond_features, dropout=dropout)

    def forward(self, x, pos, cond, d_c=None):
        if d_c is None:
            raise ValueError("SpatioTemporalTransformerLayer requires d_c (depth) to be passed.")
        x = checkpoint(self.self_attn, x, pos, cond, d_c)
        x = checkpoint(self.ff, x, cond)
        return x
...
class TokenSplit(nn.Module):
    ...
    def forward(self, x, skip):
        ...
        weight = self.fac.to(x.dtype)
        return torch.lerp(skip.to(x.dtype), x, weight)
...
@dataclass
class SpatioTemporalAttentionSpec:
    d_head: int
...
class LevelSpec:
    ...
    width: int
    d_ff: int
    self_attn: Union[GlobalAttentionSpec, NeighborhoodAttentionSpec, ShiftedWindowAttentionSpec, SpatioTemporalAttentionSpec]
    dropout: float
...
class ImageTransformerDenoiserModelV2(nn.Module):
    def __init__(self, levels, mapping, in_channels, out_channels, patch_size, num_classes=0, mapping_cond_dim=0, initial_slice_hw=(112, 112)):
        super().__init__()
        ...
        for i, spec in enumerate(levels):
            ...
            elif isinstance(spec.self_attn, SpatioTemporalAttentionSpec):
                layer_factory = lambda i: SpatioTemporalTransformerLayer(spec.width, spec.d_ff, spec.self_attn.d_head, mapping.width, dropout=spec.dropout)
    ...
    def forward(self, x, sigma, aug_cond=None, class_cond=None, mapping_cond=None, d_c=None):
        # Patching
        x = x.movedim(-3, -1)
        x = self.patch_in(x)
